$(document).ready(function(){
	...
	document.onkeyup = function(data){
		if (data.which == 27){
			$.post("http://elite_homes/chestClose");
		}
	};
});
		...
		drop: function(event,ui){
			...
			if ( tInv === "invleft" ) {
				if ( origin === "invleft" ) {
					...
					$.post("http://elite_homes/populateSlot",JSON.stringify({
						item: itemData.key,
						slot: itemData.slot,
						target: target,
					...
					$.post("http://elite_homes/takeItem",JSON.stringify({
						item: itemData.key,
						slot: target,
						amount: parseInt($(".amount").val())
					}))
				...
				if ( origin === "invleft" ) {
					...
					$.post("http://elite_homes/storeItem",JSON.stringify({
						item: itemData.key,
						slot: itemData.slot,
						amount: parseInt($(".amount").val())
					}))
		...
		drop: function(event,ui){
			...
			if ( tInv === "invleft" ) {
				if ( origin === "invleft" ) {
					...
					$.post("http:/elite_homes/updateSlot",JSON.stringify({
						item: itemData.key,
						slot: itemData.slot,
						target: target,
						amount: parseInt($(".amount").val())
					...
					$.post("http://elite_homes/sumSlot",JSON.stringify({
						item: itemData.key,
						slot: target,
						amount: parseInt($(".amount").val())
					}))
...
const updateVault = () => {
	...
	$.post("http://elite_homes/requestVault",JSON.stringify({}),(data) => {
		$("#myInfos").html(`
			...
			<div id="myInfosContent">
				...
				<span><s>BAÚ:</s> ${(data.peso2).toFixed(2)} / ${(data.maxpeso2)}</span> 
			</div>
		`); //${(data.maxpeso2).toFixed(2)}
		...
		for (let x=1; x <= mySlots; x++) {
			...
			if (data.inventario[slot] !== undefined) {
				...
				const item = `<div class="item populated" style="background-image: url('http:/131.196.197.44/inventario/${v.index}.png'); background-size: 120px; background-position: center; background-repeat: no-repeat;" data-item-key="${v.key}" data-name-key="${v.name}" data-slot="${slot}">
		...
		for (let x=1; x <= inSlots; x++) {
			...
			if (nameList2[x-1] !== undefined) {
				...
				const item = `<div class="item2 populated" style="background-image: url('http:/131.196.197.44/inventario/${v.index}.png'); background-size: 120px; background-position: center; background-repeat: no-repeat;" data-item-key="${v.key}" data-name-key="${v.name}" data-slot="${slot}">
					<div id="peso">${(v.peso*v.amount).toFixed(2)}</div>
					<div id="quantity">${formatarNumero(v.amount)}x</div>
					<div id="itemname">${v.name}</div>
