	...
	if user_id then
		...
		if data then
			if vRP.tryGetInventoryItem(user_id,configfarmerd.farmer1,amount[source]) then
				local pagamento = parseInt(math.random(jobs.farmer[1],jobs.farmer[2]))
				vRP.giveMoney(user_id,parseInt(pagamento+amount[source]))
				vRP.sendemplog1(source,"farmer",parseInt(pagamento+amount[source]))
