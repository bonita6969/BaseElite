...
Citizen.CreateThread(function()
	while true do
		...
		if not emservico then
			...
			if distance <= 3 then
				sleep = 5
				DrawText3D(configtaxi.taxiloc.x,configtaxi.taxiloc.y,configtaxi.taxiloc.z, textotaxista)
				-- DrawMarker(21,configtaxi.taxiloc.x,configtaxi.taxiloc.y,configtaxi.taxiloc.z-0.6, 0, 0, 0, 0, 0, 0, 0.7, 0.7, 0.5, 222, 67, 11, 240, 0, 0, 0, 0)
				if distance <= 1.2 then
					sleep = 5
					-- drawTexts("PRESSIONE  E  PARA INICIAR O SERVIÇO",4,0.5,0.92,0.35,255,255,255,180)
...
Citizen.CreateThread(function()
	while true do
		...
		if emservico then
			...
			if distance <= 50.0 and IsVehicleModel(vehicle,GetHashKey(configtaxi.cartaxi)) then
				idle = 5
						...
						if DoesEntityExist(passageiro) then
							while true do
								...
								if not IsPedSittingInVehicle(passageiro,vehicle) then
									DrawMarker(21,x2,y2,z2+1.3,0,0,0,0,180.0,130.0,0.6,0.8,0.5,222, 67, 11,50,1,0,0,1)
								end
